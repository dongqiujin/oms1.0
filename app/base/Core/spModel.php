	...
	public function findAll($conditions = null, $sort = null, $fields = null, $limit = null)
	{
		...
		if(is_array($conditions)){
            ...
            foreach( $conditions as $key => $condition ){
                $condition = !strstr($key, 'IN')  ? $this->__val_escape($condition) : $condition;
                if (strstr($key, '|')) {
                    $keyTmp = explode('|', $key);
                    if ($keyTmp[1] == 'like') {
                        $join[] = "{$keyTmp[0]} like '".str_replace('\'','',$condition)."%'";
                    } else {
                        $join[] = "{$keyTmp[0]} {$keyTmp[1]} {$condition}";
                    }
                } else {
                    $join[] = "{$key} = {$condition}";
                }
		...
		if( in_array( $func_name, $this->methods ) && FALSE != $this->enabled ){
			...
			if( null != $this->model_obj->linker && is_array($this->model_obj->linker) ){
				foreach( $this->model_obj->linker as $linkey => $thelinker ){
                    ...
                    // 增加APP参数,指定调用对应APP模型
                    if (isset($thelinker['app'])){
                        $GLOBALS['G_SP']['model_path'] = APP_PATH.'/'.$thelinker['app'].'/model';
                    }else{
                        $GLOBALS['G_SP']['model_path'] = APP_PATH.'/desktop/model';
                    }
