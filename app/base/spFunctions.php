...
function import($sfilename, $auto_search = TRUE, $auto_error = FALSE){
	...
	if( TRUE == @is_readable($sfilename) ){
		...
		if(TRUE == $auto_search){ // 需要搜索文件
			...
			foreach(array_merge( $GLOBALS['G_SP']['sp_include_path'], array($GLOBALS['G_SP']['model_path'],$GLOBALS['G_SP']['lib_path']), $GLOBALS['G_SP']['include_path'] ) as $sp_include_path){
				// 检查当前搜索路径中，该文件是否已经载入
 ...
 */
function spUrl($app = '', $controller = null, $action = null, $args = null, $anchor = null, $no_sphtml = FALSE) {
	...
	// 使用挂靠点
	if( $launch = spLaunch("function_url", array('app'=>$app, 'controller'=>$controller, 'action'=>$action, 'args'=>$args, 'anchor'=>$anchor, 'no_sphtml'=>$no_sphtml), TRUE ))return $launch;
	if( TRUE == $GLOBALS['G_SP']['url']["url_path_info"] ){ // 使用path_info方式
		if ($app){
			$url = $GLOBALS['G_SP']['url']["url_path_base"]."/{$app}/{$controller}/{$action}";
		}else{
			$url = $GLOBALS['G_SP']['url']["url_path_base"]."/{$controller}/{$action}";
		}
		if(null != $args)foreach($args as $key => $arg) $url .= "/{$key}/{$arg}";
	}else{
		$url = $GLOBALS['G_SP']['url']["url_path_base"]."?app=".$app."&". $GLOBALS['G_SP']["url_controller"]. "={$controller}&";
