function $(s) {
    return typeof s === 'string' && document.getElementById(s);
}
...
function getElementsByClassName(className,oBox) {
    //适用于获取某个HTML区块内部含有某一特定className的所有HTML元素
    if(document.getElementsByClassName) {
        return (oBox || document).getElementsByClassName(className);
    }
    ...
    for (var ii = 0; ii < children.length; ii++) {
        ...
        if (oRegExp.test(child.className)) {
            elements.push(child);
        }
    }
    return elements;
...
FileUpload.prototype = {
    ...
    SetOptions: function(options) {
        this.options = { //默认值
            ...
            onLimite: function() {alert("超过文件数上传限制");}, //超过文件数限制时执行
            onSame: function() {alert("已经有相同文件");}, //有相同文件时执行
            onNotExtIn: function() {alert("只允许上传" + this.ExtIn.join("，") + "文件");}, //不是允许的后缀名时执行
            ExtOut: [], //禁止后缀名，当设置了ExtIn则ExtOut无效
            onExtOut: function() {}, //是禁止的后缀名时执行
...
function makeEditor1(id) {
    KE.show({
        ...
        afterCreate: function(id) {
            var p = $(id).parentNode;
            while(p){
                if(p.nodeName.toLowerCase() === 'form' || p.nodeName.toLowerCase() === 'body') {
                    break;
                }
                p = p.parentNode;
            }
            var btn = p.getElementsByTagName('button');
            Each(btn, function(b){
                if(b.type === 'submit'){
